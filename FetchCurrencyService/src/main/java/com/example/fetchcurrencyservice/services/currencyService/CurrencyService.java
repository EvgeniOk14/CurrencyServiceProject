package com.example.fetchcurrencyservice.services.currencyService;

import com.example.fetchcurrencyservice.exceptions.customExceptions.FailedResponseException;
 ...
 * - {@link #isSentCurrenciesValid(String sentCurrencies)}: Проверяет переданные курсы валют на доступность их на сайте стороннего API
 *
 * Исключения:
...
public class CurrencyService
{
    ...
    /**
     * Метод isSentCurrenciesValid
     * Проверяет переданные валюты на присутствие их в списке доступных на сайте валют
     * @param sentCurrencies - переданные от пользователя список валют
     *
     * @return boolean - true или false, в зависимости от того есть ли такие коды валют в списке доступных кодов валют на сайте стороннего API
     * @throws FailedResponseException - недоступны данные от стороннего API
     * **/
    public boolean isSentCurrenciesValid(String sentCurrencies)
    {
        try {

            // Получаем все курсы валют от стороннего API
            ExchangeRateResponseFromAPI response = fetchAllExchangeRates(); // получаем все доступные валюты
            Map<String, Double> availableCurrencies = response.getRates(); // извлекаем коды валют

            // Проверяем, что sentCurrencies не пустая строка
            if (sentCurrencies == null || sentCurrencies.isEmpty()) // если переданный аргумент sentCurrencies роавен null или пустой, то:
            {
                return false; // возвращаем false
            }

            // Разбиваем строку на массив кодов валют
            String[] currenciesToCheck = sentCurrencies.split(","); // создаём массив с кодами переданных валют и убираем запятые

            // Проверяем наличие каждого кода валюты в доступных валютах
            for (String currency : currenciesToCheck)  // идём по массиву с переданными валютами
            {
                currency = currency.trim(); // Убираем лишние пробелы
                if (!availableCurrencies.containsKey(currency))  // если ключи (коды доступных валют коллекции availableCurrencies) не содержит хотя бы одного из переданных кодов валют, то:
                {
                    return false; // Если хотя бы одна валюта недоступна, возвращаем false
                }
            }
            return true; // Все валюты валидны
        }
        catch (FailedResponseException ex)
        {
            logger.error("Данные из стороннего API не доступны! ");
            throw ex;
        }
    }
    /**
